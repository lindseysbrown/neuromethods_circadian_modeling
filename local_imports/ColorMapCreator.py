...
class ColorMapCreator:
    ...
    def matplotlibColorMap(self):
        ...
        Note: This for some reason gives weird results that are not on 0-1. I 
        have scaled this
        """
        ...
        RGBs = np.zeros((self.numColors, 3))
        for i,s in enumerate(scalars):
            RGBs[i,:] = self.interpolateColor(self.RGB1, self.RGB2, s)
        colorMapArray = RGBs/self.divide
        cm = mpl.colors.ListedColormap(colorMapArray/np.max(colorMapArray))
        return cm
    
    def colorMapArray3(self,RGB3):
        """ adds a third color to colormap. done by smashing it between the 
        first two :^)"""
        ...
        for i,s in enumerate(scalars):
            RGBs[i,:] = self.interpolateColor(self.RGB1, RGB3, s)
            
        colorMapArray1 = RGBs/self.divide
        
        RGBs = np.zeros((self.numColors, 3))
        
        for i,s in enumerate(scalars):
            RGBs[i,:] = self.interpolateColor(RGB3, self.RGB2, s)
        
        colorMapArray2 = RGBs/self.divide
        colorMapArray = np.vstack([colorMapArray1,colorMapArray2])
