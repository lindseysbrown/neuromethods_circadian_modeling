...
class Oscillator(object):
    ...
    def approx_y0_T(self, tout=300, burn_trans=True, tol=1e-1, ref_mol=0, 
                    trans=500):
        """
        ...
        if len(peaks) > 2:
            if np.sum(np.abs(np.diff(periods))) < tol:
